...
public class BoaHancockUltimateSkill : MonoBehaviour
{
    ...
    /// </summary>
    public void SpawnUltimateCombo(BoaHancockController owner, LayerMask targetLayer)
    {
        ...
        if (hitTarget != null)
        {
            Debug.Log($"[Ultimate] BoaHancock hit target {((MonoBehaviour)hitTarget).gameObject.name}! Initiating Ultimate Combo sequence.");
            owner.StartCoroutine(UltimateComboRoutine(owner, hitTarget, dir));
        }
        else
        {
            Debug.Log("[Ultimate] BoaHancock Ultimate missed. Ending attack.");
            owner.Invoke("AnimationEvent_EndAttack", 0.3f);
        }
    }

    private IEnumerator UltimateComboRoutine(BoaHancockController owner, IDamageable target, float dir)
    {
    ...
    private void SpawnHitEffectVisual(BoaHancockController owner, Vector3 position, int hitIndex)
    {
        // Spawns hit effect only if prefab is explicitly assigned (removes the fallback hearts visual)
    ...
    {
        Gizmos.color = new Color(1f, 0.4f, 0.7f, 0.3f);
