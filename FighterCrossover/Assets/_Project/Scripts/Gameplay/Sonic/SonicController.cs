...
public class SonicController : FighterBase
{
    [Header("--- SONIC SKILLS ---")]
    public SonicRangedSkill rangedSkill;
    public SonicUltimateSkill ultimateSkill;
    ...
    protected override void Awake()
    {
        ...
        if (rangedSkill == null)
        {
            rangedSkill = GetComponent<SonicRangedSkill>();
            if (rangedSkill == null)
            {
                rangedSkill = gameObject.AddComponent<SonicRangedSkill>();
            }
        }

        if (ultimateSkill == null)
        {
            ultimateSkill = GetComponent<SonicUltimateSkill>();
            if (ultimateSkill == null)
            {
                ultimateSkill = gameObject.AddComponent<SonicUltimateSkill>();
            }
        }
    ...
    private void LoadKeybindings()
    {
        ...
        if (System.IO.File.Exists(saveFilePath))
        {
            ...
            {
                Debug.LogWarning($"[SonicController] Failed to parse settings.json: {ex.Message}");
            }
        }
    ...
    public override void OnAttack()
    {
        ...
        {
            // Chỉ buffer nếu comboStep < 4 (chưa đến hit cuối)
            if (comboStep < 4)
            {
                comboQueued = true;
            }
    ...
    {
        // Nếu có combo buffer và chưa đến hit 4 → tiếp tục combo
    ...
    private IEnumerator ExecuteUltimateWithFlash()
    {
        ...
        if (flashSprite != null)
        {
            UltimateFlashEffect.Show(this, flashSprite, ultimateFlashSizeFactor, ultimateFlashOffset, new Color(0.1f, 0.4f, 0.9f), () =>
            {
                flashDone = true;
            });
    ...
    private Sprite LoadUltimateAvatar()
    {
        ...
        Sprite s = Resources.Load<Sprite>("Sonic/avatar/avatar_sonic");
        return s;
    }
    ...
    protected override IEnumerator DashRoutine()
    {
        ...
        float dashDist = 5.5f; // Sonic dashes slightly further!
    ...
    public override void AnimationEvent_DealDamage()
    {
        ...
        foreach (var hit in hits)
        {
            ...
            if (damageable != null)
            {
                float damage = 10f + (comboStep * 5f); // Sonic does slightly less damage but hits faster
    ...
    public Sprite LoadSonicSprite(string spriteName)
    {
        ...
        string charDir = "Assets/_Project/Characters/Sonic";
    ...
    public System.Collections.Generic.List<Sprite> LoadAllSonicSprites(string filename)
    {
        ...
        return System.Linq.Enumerable.ToList(
            System.Linq.Enumerable.OrderBy(list, s => {
                int lastUnderscore = s.name.LastIndexOf('_');
                if (lastUnderscore >= 0 && lastUnderscore < s.name.Length - 1)
                {
                    int num;
                    if (int.TryParse(s.name.Substring(lastUnderscore + 1), out num))
                    {
                        return num;
                    }
                }
