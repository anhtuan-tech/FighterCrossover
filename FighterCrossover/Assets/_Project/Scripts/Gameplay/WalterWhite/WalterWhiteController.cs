...
public class WalterWhiteController : FighterBase
{
    [Header("--- WALTER WHITE SETTINGS ---")]
    public float blizzardDuration = 2.0f;

    [Tooltip("Vị trí spawn đạn. Nếu để trống sẽ dùng offset mặc định.")]
    public Transform projectileSpawnPoint;

    [Header("--- INSPECTOR ADJUSTMENTS ---")]
    [Tooltip("Tốc độ bay của đạn.")]
    public float shurikenSpeed = 12f;

    [Tooltip("Sát thương của mỗi viên đạn tầm xa.")]
    public float shurikenDamage = 1f;
    ...
    [Tooltip("Sát thương của chiêu Ultimate.")]
    ...
    private void LoadKeybindings()
    {
        ...
        if (System.IO.File.Exists(saveFilePath))
        {
            ...
            {
                Debug.LogWarning($"[WalterWhiteController] Failed to parse settings.json: {ex.Message}");
            }
        }
    ...
    public void OnSpecial()
    {
        ...
        if (!HasMana(50f))
        {
            Debug.Log("[MANA] Walter White không đủ mana dùng Ultimate!");
            return;
        }
        ExecuteUltimate();
    }
    ...
    private IEnumerator ExecuteUltimateWithFlash()
    {
        ...
        if (flashSprite != null)
        {
            UltimateFlashEffect.Show(this, flashSprite, ultimateFlashSizeFactor, ultimateFlashOffset, new Color(0.1f, 0.8f, 0.2f), () =>
            {
                flashDone = true;
            });
...
#if UNITY_EDITOR
        string avatarPath = "Assets/_Project/Characters/WalterWhite/Ultimate.png";
    ...
    private IEnumerator SpawnProjectilesRoutine()
    {
        ...
        float spread = 0.15f; // Small vertical spread/offset

        for (int i = 0; i < 10; i++)
        {
            Vector2 spawnOffset = new Vector2(dir * 1.0f, 0.5f + Random.Range(-spread, spread));
            Vector2 spawnPos = (projectileSpawnPoint != null)
                ? (Vector2)projectileSpawnPoint.position
                : (Vector2)transform.position + spawnOffset;

            GameObject projObj = new GameObject("WalterBullet");
            projObj.transform.position = spawnPos;

            var proj = projObj.AddComponent<WalterWhiteProjectile>();
            // 10 bullets, each does 1 damage
            proj.Setup(new Vector2(dir, 0f), gameObject, targetLayer, shurikenSpeed, shurikenDamage, 2f);

            yield return new WaitForSeconds(0.06f); // Shoot every 60ms
        }
    }

    public void AnimationEvent_SpawnUltimate()
    {
        // Ultimate combo strike - deals ultimateDamage melee damage in a slightly larger radius
        if (attackHitbox == null) return;

        Collider2D[] hits = Physics2D.OverlapCircleAll(attackHitbox.position, attackRadius * 1.5f, targetLayer);
        foreach (var hit in hits)
        {
            if (hit.gameObject != gameObject)
            {
                ...
                if (damageable != null)
                {
                    damageable.TakeDamage(ultimateDamage, transform.position.x, isHeavyAttack: true);
                }
            }
        }
    }
